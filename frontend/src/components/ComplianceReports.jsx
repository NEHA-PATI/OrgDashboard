...
const pieData = [
  { name: "Environmental", value: 92, fill: "#3b82f6" },
  { name: "Social", value: 91, fill: "#10b981" },
  { name: "Governance", value: 94, fill: "#8b5cf6" },
]
    ...
    <>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-secondary mt-1">Environmental, Social & Governance Performance</p>
        </div>
        <div className="flex items-center space-x-3">
          <select className="input p-2 border border-gray-300 rounded-lg" value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)}>
          ...
          <button className="button button-outline flex items-center p-2">
            <FaDownload className="btn-icon" /> <span className="ml-2">Export</span>
          </button>
          <button className="button button-outline flex items-center p-2">
            <FaFilter className="btn-icon" /> <span className="ml-2">Filter</span>
          </button>
        </div>
      </div>
      ...
      <div className="chart-section">
        <div className="chart-tabs">
          <button className={`tab ${chartType === "line" ? "active" : ""}`} onClick={() => setChartType("line")}>
            <FaChartLine className="tab-icon" /> Trends
          </button>
          <button className={`tab ${chartType === "bar" ? "active" : ""}`} onClick={() => setChartType("bar")}>
            <FaChartBar className="tab-icon" /> Compare
          </button>
          <button className={`tab ${chartType === "pie" ? "active" : ""}`} onClick={() => setChartType("pie")}>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <p className="card-description">Track your ESG metrics over time and identify trends</p>
          </div>
          <div className="card-content">
            {chartType === "line" && (
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={monthlyData}>
                  ...
                  <Line
                    ...
                    name="Responsibility"
                  />
                  <Line type="monotone" dataKey="ethics" stroke="#6366f1" strokeWidth={2} name="Ethics" />
                  <Line type="monotone" dataKey="humanRights" stroke="#ef4444" strokeWidth={2} name="Human Rights" />
                  <Line type="monotone" dataKey="stakeholder" stroke="#f97316" strokeWidth={2} name="Stakeholder" />
                  <Line type="monotone" dataKey="inclusive" stroke="#14b8a6" strokeWidth={2} name="Inclusive" />
                  <Line type="monotone" dataKey="consumer" stroke="#ec4899" strokeWidth={2} name="Consumer" />
                </LineChart>
              </ResponsiveContainer>
            )}
            ...
            {chartType === "bar" && (
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={[monthlyData[monthlyData.length - 1]]}>
                  ...
                  <Bar dataKey="environmental" fill="#3b82f6" name="Environmental" />
                  <Bar dataKey="responsibility" fill="#8b5cf6" name="Responsibility" />
                  <Bar dataKey="ethics" fill="#6366f1" name="Ethics" />
                  <Bar dataKey="humanRights" fill="#ef4444" name="Human Rights" />
                  <Bar dataKey="stakeholder" fill="#f97316" name="Stakeholder" />
                  <Bar dataKey="inclusive" fill="#14b8a6" name="Inclusive" />
                  <Bar dataKey="consumer" fill="#ec4899" name="Consumer" />
                </BarChart>
              </ResponsiveContainer>
            )}
              ...
              <div className="card-header">
                <div className="metric-header">
                  <div
                    ...
                    style={{ backgroundColor: metric.backgroundColor, color: metric.color }}
                  >
                    <IconComponent className="metric-icon" />
                  </div>
                  <span className={`badge ${getBadgeClass(metric.value)}`}>{getScoreBadge(metric.value)}</span>
                </div>
              ...
              <div className="card-content">
                <div className="metric-content">
                  <div className="metric-score-row">
                    <span className="metric-score" style={{ color: getScoreColor(metric.value) }}>
                      {metric.value}
                    </span>
